class ChatDetailConstants {
  static const String title = 'Chat';
  static const String createNewChat = 'Create new Chat';
  static const String createNewGroup = 'Create new Group';
  static const String addMembers = 'Add members';
  static const String deleteMembers = 'Delete members';
  static const String members = 'Members';
  static const String delete = 'Delete group';
  static const String leave = 'Leave group';
}
